...
Yearlines <- function(Data = Intermean, Station, col){
 ...
 # else if (Station == "Olzende"){
 # mtext("OZ", cex = 2.5, line = 4)}
 # else if (Station == "Zandkreek"){
 # mtext("ZK", cex = 2.5, line = 4)}
 
  for (m in Moname) {
      Mon <- subset(Site, Date == m)                  # select corresponding Month
      with (Mon, lines(x = Mon[,"Chla"], y = Depth, type = "l", lwd = 2, col = col[as.integer(m)-2], pch = 19))
...
#  if (Station == "ZK"){
# legend("bottomright", col = col, pch = 19, legend = c("March", "April", "May", "June", "July", "August", "September", "October", "November", "December","January 2020", "February"), cex = 1.5,
#            bty = "n") }
}
Stats <- unique(Intermean$Site)
Dat <- length(unique(Intermean$Date))
...
ZKdb  <- LinMOd(ZK, FALSE,  maxD = rep(4, times = 14)) #need to fiz ZK for some reason the columns are not being selected 
